...
void AudioMixer_cleanup(void)
{
	...
	// Free any remaining sound bites
	pthread_mutex_lock(&audioMutex);
	{
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (soundBites[i].pSound != NULL) {
...
static void fillPlaybackBuffer(short *buff, int size)
{
...

	{
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (soundBites[i].pSound != NULL) {
				int samplesToMix = MIN(soundBites[i].pSound->numSamples - soundBites[i].location, size);
				for (int j = 0; j < samplesToMix; j++) {
					// Mixing audio - ensuring not to overflow or underflow
					int mixedValue = buff[j] + soundBites[i].pSound->pData[soundBites[i].location + j];
					if (mixedValue > SHRT_MAX) {
						buff[j] = SHRT_MAX;
					} else if (mixedValue < SHRT_MIN) {
						buff[j] = SHRT_MIN;
					} else {
						buff[j] = (short)mixedValue;
					}
				}
				soundBites[i].location += samplesToMix;

				// Check if sound bite has finished playing
...
void* playbackThread(void* arg)
{
	...
	while (!stopping) {
		...
		if (frames > 0 && frames < (int)playbackBufferSize) {
